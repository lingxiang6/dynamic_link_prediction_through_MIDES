...
# 使用argparse处理命令行参数
parser = argparse.ArgumentParser(description='MIDES model')
...
parser.add_argument('--embedding_name', type=str, default='qwen2.5-7b', help='embedding name')
parser.add_argument('--feature', type=str, default='qwen', help='embedding method,[qwen, bert]')
parser.add_argument('--embedding_path_qwen', type=str, default='dynamic_link_prediction_through_MIDES/dataset/embedding_feature_qwen.npy', help='text features embedded by the qwen model')
parser.add_argument('--embedding_path_bert', type=str, default='dynamic_link_prediction_through_MIDES/dataset/embedding_feature_bert.npy', help='text features embedded by the qwen model')
parser.add_argument('--adj17_path', type=str, default='dynamic_link_prediction_through_MIDES/adjacency_matrix/clean_adj17.csv', help='path to adjacency matrix file')
parser.add_argument('--a15_path', type=str, default='dynamic_link_prediction_through_MIDES/adjacency_matrix/a15.csv', help='path to adjacency matrix file')
parser.add_argument('--industry_path', type=str, default='dynamic_link_prediction_through_MIDES/dataset/industry_matrix.pkl', help='path to industry_matrix file')
parser.add_argument('--node_index', type=str, default='dynamic_link_prediction_through_MIDES/adjacency_matrix/node_index.json', help='path to node_index.json file')
parser.add_argument('--seed', type=int, default=42, help='random seed')
parser.add_argument('--cuda', type=int, default=1, help='CUDA device ID')
parser.add_argument('--alpha', type=float, default=0.5, help='smoothing coefficient')
parser.add_argument('--beta', type=float, default=0.4, help='threshold')
parser.add_argument('--gamma', type=float, default=3e-3, help='regularization')
...
parser.add_argument('--location_path_qwen', type=str, default='dynamic_link_prediction_through_MIDES/dataset/embedding_distance_qwen.npy', help='path to qwen_location file')
parser.add_argument('--location_path_bert', type=str, default='dynamic_link_prediction_through_MIDES/dataset/embedding_distance_bert.npy', help='path to bert_location file')
...
class MIDES(torch.nn.Module):
    def __init__(self, in_channels, args):
        super(MIDES, self).__init__()
    ...
    def forward(self, x, edge_index, lambda_16_hat):
        ...
        supply_edges,  demand_edges = supply_edges.t(),  demand_edges.t()
        x = self.bns(x)
        x = self.linear(x)
...
def val(loder, model, args):
    ...
    with torch.no_grad():
        ...
        predicted_classes = (y_predicts >= args.beta).float()
...
if __name__ == '__main__':
    ...
    model = MIDES(in_channels=data.x.shape[1], args=args).to(device=args.device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay= args.gamma)
    train_steps = 0
    for epoch in range(args.epochs):
        ...
        predicted_classes = (train_y_predicts >= args.beta).float()
        ###计算acc
